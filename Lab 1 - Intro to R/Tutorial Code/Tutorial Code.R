  ...
  setwd("D:/Big Data Labs/Lab 1 - Intro to R/Tutorial Code")  #Replace this working directory with the directory containing the R file. 
  #get working directory
  getwd()
  ...
  str2 <- str
  str <- "hello "
  # When you execute str[0] <- "o", R will not throw an error, but it won't change anything in the string either. R will simply create a new string with the value "o", but it won't be assigned to any element because there's no 0th index in R indexing.
  str[0] <- "o"
#lksjdlkfjsl
  ...
  v6 <- charToRaw("Hello") #raw #Hex ascci for the chars
  ...
  v1[2]
  v1[c(2,4)] # index 2 and index 4 not a range :D
  v1[c(4,4)] # index 2 and index 2 not a range :D
  v1[2:5] # The end index is included <3
  v1[5:2] # Reversed
  ...
  v1!=2
  v1[v1>2]
  2%in%v1 #check if 2 is in vector v1
  9%in%v1
  ...
  list2 <- c(2,'car',TRUE)  # Vector must be of the same type so they are casted
  # to string 
  list2
  l <- list(v1,v2)
  l
